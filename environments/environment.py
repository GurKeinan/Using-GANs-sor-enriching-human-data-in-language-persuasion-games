...
from generate_using_GAN import get_GAN_Dataset
from train_GAN import GENERATOR_PATH, DISCRIMINATOR_PATH
...
class Environment:
    def __init__(self, model_path, config):
        ...
        # TODO: Our parameters
        self.gan_simulations = config['GAN_simulations']
        self.generator_repetitions = config['generator_repetitions']
        self.generate_best_out_of = config['generate_best_out_of']

        # delete the GAN files if they exist because we want to train new one for every hyperparameter combination we run
    ...
    def train(self, do_eval=True):
        ...
        if self.gan_simulations > 0:
            generated_dataset = get_GAN_Dataset(int(self.gan_simulations * len(train_dataset)),
                                                self.generator_repetitions, self.generate_best_out_of, retrain=self.config["retrain_GAN"])
            generated_data_loader = DataLoader(generated_dataset, batch_size=ENV_BATCH_SIZE, shuffle=True)
            phases.insert(0, ("Generated Data", generated_data_loader))
        ...
        self.set_train_mode()
        metrics = Metrics("ENV")
        model_vectors_and_ground_truth = []
        ...
        for epoch in range(self.config["total_epochs"]):
            ...
            print(f"# Epoch {epoch}")
            print("#" * 16)
            if self.config["online_simulation_size"] > 0 and online_sim_type in ["before_epoch", "mixed"]:  # TODO: uncomment
            ...
            for phase, dataloader in phases:
                ...
                if phase == "Online Simulation" and online_sim_type == "before_epoch":  # TODO: uncomment
                ...
                if phase != "Test":
                    self.set_train_mode()
                    if online_sim_type == "mixed":  # TODO: uncomment
                ...
                for batch in tqdm(dataloader, desc=phase):
                    ...
                    if self.use_user_vector:
                        model_vectors["user_vector"] = (
                            self.model.user_vectors[batch["user_id"].to("cpu").numpy()].to(device))
                        model_vectors["game_vector"] = (
                            self.model.game_vectors[batch["user_id"].to("cpu").numpy()].to(device))
                    if phase != "Test":
                    ...
                    # Save the vectors for the first epoch for later GAN training
                    if phase == 'Train' and self.use_user_vector and first_epoch_flag:
                        model_vectors_and_ground_truth.append((model_vectors, batch["action_taken"]))
            ...
            metrics.next_epoch()
            if first_epoch_flag:
                with open("model_vectors_and_ground_truth.pkl", "wb") as f:
                    pickle.dump(model_vectors_and_ground_truth, f)
